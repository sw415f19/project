...
The reason for not including casting from String to other data types is that this would require parsing and this is not the same as casting. One way to do parsing is for example to only accept changing a string to a int is if the string represents a integer

Since parsing strings to other data types have many practical uses but is slightly more time consuming and complex than parsing it should be included in Ezuino if there is enough time.  \\
